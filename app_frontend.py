...
# Apunta directamente al backend en producción
API_URL = os.getenv("API_URL", "https://tesis-api-fumq.onrender.com")
...
def obtener_arquitectos():
    try:
        res = requests.get(f"{API_URL}/arquitectos", timeout=30)
...
def obtener_obreros():
    try:
        res = requests.get(f"{API_URL}/obreros", timeout=30)
        if res.status_code == 200:
            return pd.DataFrame(res.json())
    except requests.exceptions.RequestException:
        pass
...
with st.sidebar:
    ...
    if st.session_state['rol_actual'] == "Arquitecto":
        ...
        if arquitectos.empty:
            st.warning("No se pudo conectar con el servidor. Intentá nuevamente en unos segundos.")
            if st.button("🔄 Reintentar"):
                obtener_arquitectos.clear()
                st.rerun()
            st.stop()
    ...
    with tab1:
        with st.form("busqueda_form"):
            ...
            if estrategia == "Balanceada": 
                p_nlp, p_est, p_dist = 0.4, 0.4, 0.2
            elif estrategia == "Mejores Valorados": 
                p_nlp, p_est, p_dist = 0.3, 0.6, 0.1
            else: 
                p_nlp, p_est, p_dist = 0.3, 0.2, 0.5
        ...
        if submit_button:
            if not query_texto: 
                ...
                payload = {
                    ...
                    "peso_nlp": p_nlp, 
                    "peso_estrellas": p_est, 
                    "peso_distancia": p_dist, 
                    "top_n": top_n
                }
                if tipo_ubicacion == "Ingresar Manualmente" and ubicacion_manual.strip() != "": 
                    payload["ubicacion_manual"] = ubicacion_manual
                else: 
                    payload["lat_arq"], payload["lon_arq"] = -34.6037, -58.3816
                try:
                    response = requests.post(f"{API_URL}/recomendar", json=payload, timeout=15)
                    if response.status_code == 200: 
                        st.session_state['datos_busqueda'] = response.json()
                    else: 
            ...
            if not obreros: 
                ...
                if "lat" in ubicacion and "lon" in ubicacion:
                    ...
                    for obs in obreros:
                        if "ultima_latitud" in obs and "ultima_longitud" in obs:
                            ...
                            folium.Marker(
                                [obs["ultima_latitud"], obs["ultima_longitud"]], 
                ...
                for ob in obreros:
                    ...
                    if st.button(f"Enviar Oferta", key=f"btn_{ob['id_usuario']}"):
                        res = requests.post(f"{API_URL}/ofertar", json={
                            "id_arquitecto": st.session_state['usuario_id'], 
                            "id_obrero": ob['id_usuario']
                        }, timeout=15)
                        if res.status_code == 200: 
                            st.success("¡Oferta enviada con éxito!")
                        else: 
                            st.error(res.json().get('detail', "Error al enviar la oferta"))
    with tab2:
        st.subheader("📋 Mis Contrataciones")
        if "contratos" not in st.session_state:
            res = requests.get(f"{API_URL}/contratos/arquitecto/{st.session_state['usuario_id']}", timeout=15)
            if res.status_code == 200:
...
# ==========================================
    ...
    if st.button("📍 Compartir mi ubicación actual"):
        requests.post(f"{API_URL}/actualizar_ubicacion", json={
            "id_obrero": st.session_state['obrero_id'],
            "latitud": -34.6100,
            "longitud": -58.4000
        }, timeout=15)
    ...
    if "ofertas_obrero" not in st.session_state:
        res = requests.get(f"{API_URL}/contratos/obrero/{st.session_state['obrero_id']}", timeout=15)
        if res.status_code == 200:
            ...
            for c in contratos:
                ...
                if c['estado_obra'] == "Pendiente":
                    ...
                    if col1.button("✅ Aceptar Oferta", key=f"acep_{c['id_contrato']}"):
                        r = requests.post(f"{API_URL}/cambiar_estado", json={
                            "id_contrato": c['id_contrato'],
